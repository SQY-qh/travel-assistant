...
const shanghaiRecommendation = {
  ...
  matchReason: '已为“深圳到上海 / 7 月 1 日出发 / 5 天 / 情侣 / 预算 1 万”命中本地预存方案，不调用远程模型或实时价格接口。',
  coverImage: destinationCoverImage('上海'),
...
const dayPlans: DayPlan[] = [
  {
    ...
    spots: [
      ...
      { time: '12:30', name: '人民广场 / 南京东路酒店入住', address: '人民广场、南京东路、苏州河沿线', type: '酒店', note: '建议住人民广场到南京东路之间，去外滩、武康路、迪士尼和虹桥都比较均衡；预算按 4 晚 ¥3000-3600 控制。', lat: 31.2355, lng: 121.4752, cost: 3200 },
      { time: '15:30', name: '南京东路步行街', type: '景点', note: '首日下午只做轻量城市适应，逛第一食品商店、和平饭店外观、外滩源一带；人多时走支路到圆明园路更舒服。', lat: 31.2397, lng: 121.4902, cost: 0 },
      { time: '18:00', name: '外滩日落到蓝调时刻', type: '景点', note: '情侣照建议 18:30 前到黄浦公园附近占位，先拍万国建筑群，再等陆家嘴亮灯；晚餐不要排太远，避免错过夜景。', lat: 31.2405, lng: 121.4909, cost: 0 },
      { time: '20:00', name: '北外滩滨江 / 乍浦路桥', type: '景点', note: '外滩人多时转去北外滩，机位更开阔；乍浦路桥适合拍东方明珠和苏州河夜色。', lat: 31.2503, lng: 121.4985, cost: 0 },
    ],
  },
  {
    day: 2,
    ...
    spots: [
      { time: '09:00', name: '武康大楼', type: '景点', note: '早上 9 点前人相对少，拍完经典转角不要久停；之后沿武康路向安福路走，街区比单点更值得慢逛。', lat: 31.2103, lng: 121.4376, cost: 0 },
      { time: '10:30', name: '安福路与话剧艺术中心周边', type: '景点', note: '适合咖啡、买小众香氛和拍街景；如果太阳太晒，就把户外压缩到 60 分钟，转进店内休息。', lat: 31.2152, lng: 121.4442, cost: 120 },
      { time: '13:00', name: '湖南路 / 永福路午餐', type: '餐饮', note: '推荐选 brunch 或本帮菜小馆，避开热门网红店长队；两人午餐按 ¥220-300 预算。', lat: 31.2114, lng: 121.4446, cost: 260 },
      { time: '15:00', name: '徐家汇书院', type: '景点', note: '下午最热时放室内，书院空间好拍，也能休整；若遇雨可延长到 2 小时。', lat: 31.1919, lng: 121.4399, cost: 0 },
  ...
  },
  {
    ...
    spots: [
      ...
      { time: '09:00', name: '上海迪士尼乐园', type: '景点', note: '情侣优先顺序：疯狂动物城热力追踪、创极速光轮、加勒比海盗、七个小矮人矿山车。只选 4-5 个重点，不必硬刷全园。', lat: 31.144, lng: 121.657, cost: 1180 },
      { time: '13:30', name: '园内午餐与降温休息', type: '餐饮', note: '中午避开排队和暴晒，选室内餐厅；带小风扇、雨衣和水杯，预算两人 ¥180-260。', lat: 31.144, lng: 121.657, cost: 220 },
      { time: '18:30', name: '城堡烟花 / 夜间巡游', type: '景点', note: '如果当日烟花开放，提前 45 分钟找位置；若雨天取消，改去迪士尼小镇晚餐后回酒店。', lat: 31.144, lng: 121.657, cost: 0 },
  ...
  },
  {
    ...
    spots: [
      ...
      { time: '15:00', name: '苏河湾万象天地 / 天后宫桥', type: '景点', note: '下午走苏州河沿线，商场、河岸、历史建筑组合稳定；雨天也能在室内完成大部分体验。', lat: 31.2468, lng: 121.4716, cost: 0 },
      { time: '17:30', name: '静安寺外观与久光商圈', type: '景点', note: '傍晚去静安寺外观更有城市反差感；如果想购物，久光、芮欧、晶品都在步行范围。', lat: 31.223, lng: 121.4452, cost: 100 },
  ...
  },
  {
    ...
    spots: [
      { time: '09:30', name: '愚园路城市更新街区', type: '景点', note: '最后一天不排重景点，适合咖啡、买伴手礼、补拍街景；比外滩更适合作为轻松收尾。', lat: 31.2209, lng: 121.4317, cost: 80 },
...
export const shenzhenShanghaiBookingComparison: BookingComparison = {
  title: '深圳到上海前后一天整体比价',
  baseline: '按 2 人、4 晚、上海市中心舒适型酒店估算；为本地预存攻略价，不调用实时票价接口。',
...
export function buildShenzhenShanghaiLocalPlan(profile: TravelProfile): TravelPlan {
  return {
    ...
    policyCards: [
      { title: '本地预存方案', summary: '该深圳到上海方案已内置在前端本地，不依赖远程模型、机酒火车票或地图价格接口生成。', level: '重要' },
    ...
    notes: [
      ...
      '机票、火车票、酒店价格为本地预存攻略价，用于展示和决策，不代表实时库存。',
